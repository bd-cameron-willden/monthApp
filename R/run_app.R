#' Run the app
#'
#' Initiate an instance of the 'Omnibus Normality' app. If the current context is interactive,
#' (i.e. if the function is called from RStudio), this function will initiate the app stored in
#' inst/app with the runApp function. If the context is not interactive (i.e. the project is being sourced in Posit
...
run_app <- function() {
  ...
  if (interactive()) {
    ...
    runApp(appDir = system.file("app",
                                package = "monthApp")) #this goes to inst/app directory to run the app
